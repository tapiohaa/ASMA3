...
#           by estimating difference-in-differences (DD) models using 
...
# Outputs:
output = 'W:/ASMA3/data/cleaned/intro_results_cs.rds'
output_dyn_cs_pri_short = 
  "W:/ASMA3/analysis/figures/intro_dynamic_plot_cs_pri_short.pdf"
output_dyn_cs_sec_short = 
  "W:/ASMA3/analysis/figures/intro_dynamic_plot_cs_sec_short.pdf"
output_dyn_cs_all_short = 
  "W:/ASMA3/analysis/figures/intro_dynamic_plot_cs_all_short.pdf"
output_dyn_cs_pri_log =
  "W:/ASMA3/analysis/figures/intro_dynamic_plot_cs_pri_log.pdf"
output_dyn_cs_pri_long = 
  "W:/ASMA3/analysis/figures/intro_dynamic_plot_cs_pri_long.pdf"
output_dyn_cs_ter = 
  "W:/ASMA3/analysis/figures/intro_dynamic_plot_cs_ter_long.pdf"
output_att_plot_cs_pri = "W:/ASMA3/analysis/figures/intro_att_plot_cs_pri.pdf"
output_att_plot_cs_pri_log = 
  "W:/ASMA3/analysis/figures/intro_att_plot_cs_pri_log.pdf"
output_att_plot_cs_sec = "W:/ASMA3/analysis/figures/intro_att_plot_cs_sec.pdf"
output_att_plot_cs_all = "W:/ASMA3/analysis/figures/intro_att_plot_cs_all.pdf"
output_attevent_plot_cs_pri = 
  "W:/ASMA3/analysis/figures/intro_attevent_plot_cs_pri.pdf"
output_att_plot_cs_ter = "W:/ASMA3/analysis/figures/intro_att_plot_cs_ter.pdf"
...
estimate.cs = function(data, controls) {
  ...
  # Aggregate group-time ATTs to cohort-specific effects:
  agg.grp = did::aggte(reg, type = 'group')
  
  # Collect cohort-specific results to a table:
  
  relative_time = agg.grp$egt
  estimate = agg.grp$att.egt
  std_error = agg.grp$se.egt
  conf_low = estimate - qnorm(0.05/2,lower=F) * std_error
  conf_high = estimate + qnorm(0.05/2,lower=F) * std_error
  
  table.grp = data.table(
    relative_time=relative_time,
    estimate=estimate, std_error=std_error,
    conf_low=conf_low, conf_high=conf_high,
    event_study = 2
  ...
  table = rbind(table.att, table.es, table.grp, fill=TRUE)
  return(table)
...
# For tertiary outcomes, we loop over visit type (ED / inpatient), 
# comparison group, outcome, start year, and end year:
...
specs_ter = CJ(visit_types, controls, start_years, end_years, outcomes)

# Add specification numbers to controls-start_year combinations:
spec_no = CJ(controls, start_years, end_years)
spec_no[, spec_no := c(1:nrow(spec_no))]
specs_ter = merge(specs_ter, spec_no, by=c('controls', 'start_years',
                                           'end_years'), all.x=TRUE)
...
cs.ter = results.cs(data=df, models = models_sec, specs = specs_ter)
saveRDS(rbind(cs.pri, cs.sec, cs.ter, fill=TRUE), output)
...
#cs.ter = cs[outcome %in% c('contacts_per_capita', 'contacts_per_capita_log') &
#              visit_type %in% c(50, 60)]
...
plot.es = function(data, otc, short_d=1, start_yr, all.p=0) {
  # INPUTS:
  # data: either cs.pri or cs.sec
  # otc: 'contacts_per_capita', 'contacts_per_capita_log', 
  #       'social_assistance', or 'basic_social_assistance'
  ...
  # all.p: a dummy for whether we want to include all individuals ('all_pc')
  # OUTPUTS:
  # a ggplot object
  ...
  DT = data[event_study==1 & controls=='notyettreated' & 
              outcome==otc & start_year==start_yr & end_year==end_yr]
  
  if(all.p==1) { DT = DT[mod %in% c('all_pc', 'all')]}
  else if(all.p==0) { DT = DT[!(mod %in% c('all_pc', 'all'))]}
  ...
  # If our outcome is 'contacts_per_capita' or 'contacts_per_capita_log' 
  # (in primary care), we will plot a faceted plot. To this end, order the 
  # levels of 'profession' to edit the order of facets:
  if(otc %in% c('contacts_per_capita', 'contacts_per_capita_log')) {
    DT[visit_type == 2, profession := 'Nurse visits']
    DT[visit_type == 1, profession := 'GP visits']
    DT[visit_type == 50, profession := 'ED visits']
    DT[visit_type == 60, profession := 'ACSC inpatient admissions from ED']
    DT$profession_f = factor(DT$profession, 
                             levels=c('Nurse visits', 'GP visits', 'ED visits', 
                                      'ACSC inpatient admissions from ED'))
  }
  ...
  if(short_d==0 & otc != 'basic_social_assistance') {
    
    p = p + geom_rect(xmin=-Inf, xmax=-24.5, ymin=-Inf, ymax=Inf, 
                      fill='grey45', alpha=0.005) +
      geom_rect(xmin=11.5, xmax=Inf, ymin=-Inf, ymax=Inf, 
                fill='grey45', alpha=0.005) +
      geom_vline(xintercept = c(-24.5, 11.5)) 
  ...
  if(otc %in% c('contacts_per_capita', 'contacts_per_capita_log') & 
     (all.p==0 | (all.p==1 & 
                  identical(DT[, sort(unique(visit_type))], c(50,60))) ) ) {
    p = p + facet_grid(profession_f ~ mod, scales='free') 
  } else if(otc %in% c('contacts_per_capita', 'contacts_per_capita_log') & 
            all.p==1) {
    p = p + facet_grid(~ profession_f, scales='free') }
...
p2 = plot.es(data=cs.sec, otc='social_assistance', start_yr=2013, all.p=1) +
  ggtitle('A. Share receiving social assistance')
p3 = plot.es(data=cs.sec, otc='basic_social_assistance', start_yr=2013, 
             all.p=1,) + ggtitle('B. Sum of basic social assistance')
p4 = plot.es(data=cs.pri, otc='contacts_per_capita', start_yr=2014, all.p=1)
p5 = plot.es(data=cs.pri, otc='contacts_per_capita_log', start_yr=2014)
p6 = plot.es(data=cs.pri, otc='contacts_per_capita', start_yr=2014, short_d = 0)
p7 = plot.es(data=cs.ter, otc='contacts_per_capita', 
             start_yr=2014, short_d=0, all.p=1)
p8 = plot.es(data=cs.ter, otc='contacts_per_capita_log', 
             start_yr=2014, short_d=0, all.p=1)

# Save:

cairo_pdf(filename = output_dyn_cs_pri_short, width = 15, height = 10)
print(p1) 
dev.off()

cairo_pdf(filename = output_dyn_cs_sec_short, width = 15, height = 6)
print(p2 + p3) 
dev.off()

cairo_pdf(filename = output_dyn_cs_all_short, width = 15, height = 6)
print(p4) 
dev.off()

cairo_pdf(filename = output_dyn_cs_pri_log, width = 15, height = 10)
print(p5) 
dev.off()

cairo_pdf(filename = output_dyn_cs_pri_long, width = 15, height = 10)
print(p6) 
dev.off()

cairo_pdf(filename = output_dyn_cs_ter, width = 15, height = 10)
print(p7 + p8) 
dev.off()
...
plot.att = function(data, otc, base_spec, all.p=0) {
  # INPUTS:
  # data: either cs.pri or cs.sec
  ...
  p = ggplot(DT, aes(x=spec_no, y=estimate, label=N)) +
    geom_point(data=DT[spec_no!=base_spec], color='grey40') +
    geom_point(data=DT[spec_no==base_spec], shape=15, size=3) +
    geom_pointrange(data=DT[spec_no!=base_spec], 
                    aes(ymin=conf_low, ymax=conf_high), color='grey40') +
    geom_pointrange(data=DT[spec_no==base_spec], 
...
p3 = plot.att(data=cs.sec, otc='social_assistance', base_spec=2, all.p=1) +
  ggtitle('A. Share receiving social assistance')
p4 = plot.att(data=cs.sec, otc='basic_social_assistance', base_spec=2,all.p=1) +
  ggtitle('B. Sum of basic social assistance')
p5 = plot.att(data=cs.pri, otc='contacts_per_capita', base_spec=7, all.p = 1)
p6 = plot.att(data=cs.pri, otc='contacts_per_capita_log', base_spec=7, all.p= 1)
p7 = plot.att(data=cs.ter, otc='contacts_per_capita', base_spec=7, all.p= 1)
p8 = plot.att(data=cs.ter, otc='contacts_per_capita_log', base_spec=7, all.p= 1)


# Save:

cairo_pdf(filename = output_att_plot_cs_pri, width = 15, height = 10)
print(p1) 
dev.off()

cairo_pdf(filename = output_att_plot_cs_pri_log, width = 15, height = 10)
print(p2) 
dev.off()

cairo_pdf(filename = output_att_plot_cs_sec, width = 15, height = 6)
print(p3 + p4) 
dev.off()

cairo_pdf(filename = output_att_plot_cs_all, width = 15, height = 10)
print(p5 + p6 + plot_layout(ncol=1)) 
dev.off()

cairo_pdf(filename = output_att_plot_cs_ter, width = 15, height = 10)
print(p7 + p8) 
dev.off()


### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 3.3: Event-specific effects. ####
### ### ### ### ### ### ### ### ### ### ### ### ###


### A function that plots the event-specific effects. ###

plot.event.att = function(data, otc, start_yr, all.p=0) {
  # INPUTS:
  # data: either cs.pri or cs.sec
  # otc: 'contacts_per_capita', 'contacts_per_capita_log', 
  #       'social_assistance', or 'basic_social_assistance'
  ...
  if(otc %in% c('contacts_per_capita', 'contacts_per_capita_log')) {
    ...
    print('The outcome is currently not supported')
    return()
  }
  
  # Extract data:
  DT = data[event_study==2 & controls=='notyettreated' & 
              outcome==otc & start_year==start_yr & end_year==end_yr]
  DT.att = data[event_study==0 & controls=='notyettreated' & 
                   outcome==otc & start_year==start_yr & end_year==end_yr]
  ...
  # Transform relative time to dates:
  
  dates = CJ(year=c(2012:2019), month=c(1:12))
  dates[, ':=' (eventdate = as.Date(paste(year, month, '1', sep = '-')),
                relative_time = c(1:96))]
  dates = dates[, .(eventdate, relative_time)]
  
  DT = merge(DT, dates, by='relative_time', all.x = TRUE)
  
  
  # As our outcome is 'contacts_per_capita' or 'contacts_per_capita_log' 
  # (in primary care), we plot a faceted plot. To this end, order the 
  # levels of 'profession' to edit the order of facets:
  if(otc %in% c('contacts_per_capita', 'contacts_per_capita_log')) {
    DT[visit_type == 2, profession := 'Nurse visits']
    DT[visit_type == 1, profession := 'GP visits']
  ...
  # Axis titles:
  if(otc=='contacts_per_capita') { y_lab = 'Ann. visits per capita' } 
  else if (otc=='contacts_per_capita_log') {y_lab='Log. ann. visits per capita'} 
  
  
  # Plot:
  
  p = ggplot(DT, aes(x=eventdate, y=estimate)) +
    geom_point() +
    geom_segment(aes(x=eventdate, xend=eventdate, y=conf_low, yend=conf_high)) +
    stat_smooth(method = 'lm', se=FALSE, aes(colour='OLS fit')) + 
    labs(x = 'Adoption (event) date', y = y_lab) +
    scale_colour_manual(name='Method', values = 'black') + 
    theme(text = element_text(size=20),   
          axis.text.x = element_text(hjust = 1),
          panel.background = element_rect(fill = "white", colour = "white"),
          panel.grid.major = element_line(size = 0.25, linetype = "solid", 
...
p7 = plot.event.att(data=cs.pri, otc='contacts_per_capita', start_yr=2014)

cairo_pdf(filename = output_attevent_plot_cs_pri, width = 15, height = 11)
print(p7) 
dev.off()
