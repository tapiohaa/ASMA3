...
# Outputs:
output_summary_table = "W:/ASMA3/analysis/tables/intro_summary_table"
output_intro = "W:/ASMA3/data/interim/adoption_policies.rds"
output_abol = "W:/ASMA3/data/interim/abolition_policies.rds"
output_plot_adopt = "W:/ASMA3/analysis/figures/copay_policies_adopt.pdf"
output_plot_abol = "W:/ASMA3/analysis/figures/copay_policies_abolition.pdf"
...
### A table showing the characteristics of never-treated, earlier-treated,
# and later-treated municipalities. ###

# Define later-treated municipalities as those whose event is after the median:
policies[is.na(nurse_copay_date), group := 'never-treated']
policies[nurse_copay_date <= median(policies$nurse_copay_date, na.rm=TRUE), 
         group := 'earlier-treated']
policies[nurse_copay_date > median(policies$nurse_copay_date, na.rm=TRUE), 
         group := 'later-treated']

groups = policies[, .(municipality_code, group)]

# Read and select several municipal characteristics from 2013:
df_muni = as.data.table(readRDS("W:/ASMA3/data/raw/municipal_data.rds"))
df_muni = 
  df_muni[year==2013, 
          mget(c('no', "population", "population_change_from_the_previous_year_percent", "degree_of_urbanisation_percent", 
                 "employment_rate_percent",
                 "share_of_persons_aged_15_or_over_with_at_least_upper_secondary_qualifications_percent",
                 "proportion_of_pensioners_of_the_population_percent", 
                 "nurse_visits_per_capita"))]

# Merge:
groups = merge(groups, df_muni, by.x='municipality_code', by.y='no', all.x=TRUE)

# Compute population-weighted means:
groups = 
  groups[, .('Population' = mean(population),
             'Population change, percent' = 
               weighted.mean(population_change_from_the_previous_year_percent, 
                             w=population),
             'Degree of urbanization' = 
               weighted.mean(degree_of_urbanisation_percent, w=population),
             'Upper secondary education' = 
               weighted.mean(share_of_persons_aged_15_or_over_with_at_least_upper_secondary_qualifications_percent, 
                             w=population),
             'Employment rate' = 
               weighted.mean(employment_rate_percent, w=population),
             'Share of pensioners' = 
               weighted.mean(proportion_of_pensioners_of_the_population_percent, 
                             w=population),
             'Nurse visit per capita' =  
               weighted.mean(nurse_visits_per_capita, w=population)), 
         by='group']

# Pivot longer and wider:
col_names = setdiff(colnames(groups), 'group')
groups = dcast(melt(groups, id.vars = 'group', measure.vars = col_names),
               variable ~ group, value.var = 'value')

# Round to 3 digits:
col_names = c('never-treated', 'earlier-treated', 'later-treated')
groups[, (col_names) := round(.SD, digits=3), .SDcols = col_names]
...
save.table(groups, output=output_summary_table,
  label_tex = 'tab:intro_summary_table',
  title_tex='Adoption: Summary Stats of the Never-Treated, Earlier-Treated, and Later-Treated.'
)
...
plot.copay = function(data, outcome, abolition=0) {
  ...
  df[treat==0, treat_char := 'No']
  df[treat==1, treat_char := 'Yes']
  df$treat_char = factor(df$treat_char, levels = c('Yes', 'No'))
  ...
  # Plot:
  p = ggplot(df, aes(x=date, y=outcome, linetype=treat_char, group=treat_char, 
                     color=treat_char)) + 
    ylab(ylab_title) +
    scale_y_continuous(breaks = brks, limits = y_lim) + 
    scale_x_date(date_breaks = '1 year', date_labels = '%Y') +
    scale_color_manual(values=c('black', 'grey40')) +
    labs(linetype='Has a nurse visit copayment', 
         group='Has a nurse visit copayment', 
         color='Has a nurse visit copayment') +
  ...
  if(abolition == 0) {
    p = p + geom_step(size=1.5)
  } else if (abolition==1) {
    p = p + geom_line(na.rm = TRUE, size=1.5) +
...
cairo_pdf(filename = output_plot_adopt, width = 15, height = 5.5)
print(p1 + ggtitle('Municipalities.') + 
        p2 + ggtitle('Population.') +
        plot_layout(guides = "collect") & 
        theme(legend.position='bottom'))
dev.off()

cairo_pdf(filename = output_plot_abol, width = 15, height = 5.5)
print(p3 + ggtitle('Municipalities.') + 
        p4 + ggtitle('Population.') + 
        plot_layout(guides = "collect") & 
        theme(legend.position='bottom'))
dev.off()
...
cairo_pdf(filename = output_policies, width = 15, height = 7.5)
...
print(ggplot() +
        ...
        scale_fill_viridis(name = 'Has a nurse visit copayment',
                           discrete = TRUE) +
        coord_sf(datum = NA) + 
        theme(text = element_text(size=20),   
              panel.background = element_rect(fill = "white", colour = "white"),
...
p.abol.2 = ggplot() +
  geom_sf(data=DT$geom, fill="white") +
  geom_point(data=DT, aes(size=population, geometry=geom, fill=policy.f), 
             col = 'black', stat = 'sf_coordinates', pch=21) + 
  scale_fill_viridis(name = 'Has a nurse visit copayment',
                      discrete = TRUE, guide='none') +
...
cairo_pdf(filename = output_map_abol, width = 12, height = 12)
