...
output_intro_copay_gp = 
  "W:/ASMA3/analysis/unweighted/figures/uw_intro_trends_plot_copay_gp.pdf"
output_intro_nurse = 
  "W:/ASMA3/analysis/unweighted/figures/uw_intro_trends_plot_nurse.pdf"
output_intro_gp = 
  "W:/ASMA3/analysis/unweighted/figures/uw_intro_trends_plot_gp.pdf"
output_intro_sa = 
  "W:/ASMA3/analysis/unweighted/figures/uw_intro_trends_plot_sa.pdf"
output_intro_all = 
  "W:/ASMA3/analysis/unweighted/figures/uw_intro_trends_plot_all.pdf"
output_abol_nurse = 
  "W:/ASMA3/analysis/unweighted/figures/uw_abol_trends_plot_nurse.pdf"
output_abol_gp = 
  "W:/ASMA3/analysis/unweighted/figures/uw_abol_trends_plot_gp.pdf"
output_abol_drugs = 
  "W:/ASMA3/analysis/unweighted/figures/uw_abol_trends_plot_drugs.pdf"
#output_abol_refs = 
#  "W:/ASMA3/analysis/unweighted/figures/uw_abol_trends_plot_refs.pdf"
output_abol_all = 
  "W:/ASMA3/analysis/unweighted/figures/uw_abol_trends_plot_all.pdf"
...
abol.all = abol[, .(value = mean(contacts_per_capita)),
...
abol = abol[!is.na(bottom_d), .(value = mean(contacts_per_capita)),
...
specs[plot.types=='diff', title2 := 'diff. (T-C)']
...
p3 = plot.treatcontrol(intro, type='diff', income_bottom=NULL, visit_type = 2,
                       p.title = 'GP visit copayment', data_type='intro') +
  ggtitle('Difference (T-C)')


# Trend plots for the whole population:
...
print(wrap_elements(panel = p1 + p2 + plot_layout(guides='collect') & 
                      theme(legend.position = 'bottom')) + 
        wrap_elements(panel = p3 + plot_layout(guides='collect') & 
                        theme(legend.position = 'bottom')) +
        plot_layout(ncol=2, widths = c(2, 1)))
dev.off()

cairo_pdf(filename = output_intro_nurse, width = 15.0, height = 8.0)
print(wrap_elements(panel = wrap_plots(
  plots.intro$`2`[lengths(plots.intro$`2`) != 0][1:4], 
  ncol=2, byrow = FALSE, guides='collect') & 
    theme(legend.position = 'bottom')) +
    wrap_elements(panel = wrap_plots(
      plots.intro$`2`[lengths(plots.intro$`2`) != 0][5:6], 
...
print(wrap_elements(panel = wrap_plots(
  plots.intro$`1`[lengths(plots.intro$`1`) != 0][1:4], 
  ncol=2, byrow = FALSE, guides='collect') & 
    theme(legend.position = 'bottom')) +
    wrap_elements(panel = wrap_plots(
      plots.intro$`1`[lengths(plots.intro$`1`) != 0][5:6], 
      ncol=1, byrow = FALSE, guides='collect') & 
        theme(legend.position = 'bottom')) +
    plot_layout(ncol=2, widths = c(2, 1))
) 
...
print(wrap_elements(panel = wrap_plots(
  c(plots.intro$`50`[lengths(plots.intro$`50`) != 0][1:2],
    plots.intro$`60`[lengths(plots.intro$`60`) != 0][1:2]),
  ncol=2, byrow=TRUE, guides='collect') &
    theme(legend.position = 'bottom')) +
    wrap_elements(panel = wrap_plots(
      c(plots.intro$`50`[lengths(plots.intro$`50`) != 0][3],
        plots.intro$`60`[lengths(plots.intro$`60`) != 0][3]),
      ncol=1, byrow=TRUE, guides='collect') &
        theme(legend.position = 'bottom')) +
    plot_layout(ncol=2, widths = c(2, 1))
) 
dev.off()
...
print(wrap_elements(
  panel = plots.intro.all$contact.2$smooth + ggtitle('Nurse visits: smoothed') + 
    ...
    wrap_elements(
      panel = plots.intro.all$contact.2$diff + ggtitle('Nurse visits: diff. (T-C)') +
        plots.intro.all$contact.1$diff + ggtitle('GP visits: diff. (T-C)') +
...
print(wrap_elements(
  panel = plots.abol.all$contact.2$smooth + ggtitle('Nurse visits: smoothed') + 
    ...
    theme(text = element_text(size=20), legend.position = 'bottom')) +
    wrap_elements(
      panel = plots.abol.all$contact.2$diff + ggtitle('Nurse visits: diff. (T-C)') +
        plots.abol.all$contact.1$diff + ggtitle('GP visits: diff. (T-C)') +
        plot_layout(ncol = 1, guides = 'collect') & 
        theme(text = element_text(size=20), legend.position = 'bottom')) +
    plot_layout(ncol = 2, widths = c(2, 1)))
dev.off()
