...
input_morbidities = "W:/ASMA3/data/interim/morbidities.csv"
...
input_hosp = "W:/ASMA3/data/interim/hospital_visits_clean.csv"
input_drugs = "W:/ASMA3/data/interim/prescriptions_" # prescriptions_X.csv, X in 1:5

# Outputs:
output_plot_sa = "W:/ASMA3/analysis/figures/social_assistance_by_deciles.pdf"
...
# Read the data on morbidities:
morb = data.table::fread(input_morbidities)
folk = merge(folk, morb, by.x='id', by.y='shnro', all.x = TRUE)
folk[is.na(morbidity), morbidity := 0]
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 2) Plot the share of social assistance recipients by decile. ####
### ### ### ### ### ### ### ### ### ### ### ### ###

df_sa = folk[year %in% c(2013:2019) & basic_social_assistance > 0, 
             .(N = .N,
               sum_euros = sum(basic_social_assistance)), 
             by='income_decile'
             ][, ':=' (Recipients = 100 * N / sum(N),
                       Euros = 100 * sum_euros / sum(sum_euros),
                       income_decile = as.integer(as.character(income_decile)) + 1)]

setnames(df_sa, old='income_decile', new='Decile')

df_sa = melt(df_sa, id.vars = 'Decile', measure.vars = c('Euros', 'Recipients'),
             variable.name = 'Variable')


p = ggplot(data=df_sa, aes(x=Decile, y=value, group=Variable, shape=Variable)) +
  geom_line() +
  geom_point(size=4) +
  ylab('Share (%)') + 
...
cairo_pdf(filename = output_plot_sa, width = 15, height = 10)
print(p)
dev.off()
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 3) Compute population size in each subgroup. ####
### ### ### ### ### ### ### ### ### ### ### ### ###

cols = c('income_decile', 'morbidity')
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 5) Merge nurse and GP visits and FOLK data and aggregate. ####
### ### ### ### ### ### ### ### ### ### ### ### ###

# Merge:
visits = merge(visits, folk, by=c('id','year'), all.x = TRUE)
print("Merge is done")
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 6) Read referrals to specialized healthcare. ####
### ### ### ### ### ### ### ### ### ### ### ### ###
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 7) Merge referrals and FOLK data and aggregate. ####
### ### ### ### ### ### ### ### ### ### ### ### ###

# Merge:
refs = merge(refs, folk, by=c('id','year'), all.x = TRUE)
print("Merge is done")
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 8) ED visits and ACSC inpatient spells. ####
### ### ### ### ### ### ### ### ### ### ### ### ###

hosp = data.table::fread(input_hosp)
hosp[, V1 := NULL]
hosp = merge(hosp, folk, by=c('id','year'), all.x = TRUE)
print("Merge is done")

# Approximately 29% of the rows are dropped because the ID cannot 
# be merged to FOLK data (we took those aged 25 or more).
colMeans(is.na(hosp))
hosp = hosp[!is.na(municipality)]
...
df_hosp = lapply(cols, function(x) {
  
  data = hosp[, .(contacts = sum(visits)), by=c('municipality', 'year', 
                                                'month', 'profession', x)
              ][, dimension := x]
  setnames(data, old=x, new='group')
...
df_hosp = do.call(rbind.data.frame, df_hosp)

# Show the number of contacts by year:
test = df_hosp[, .(contacts = sum(contacts)), 
               by=c('year','dimension','profession')][order(profession, year)]
test


### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 9) Read drug prescriptions. ####
### ### ### ### ### ### ### ### ### ### ### ### ###
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 10) Merge prescriptions and FOLK data and aggregate. ####
### ### ### ### ### ### ### ### ### ### ### ### ###

# Merge:
drugs = merge(drugs, folk, by=c('id','year'), all.x = TRUE)
print("Merge is done")
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 11) Merge data on social assistance and FOLK data and aggregate. ####
### ### ### ### ### ### ### ### ### ### ### ### ###
...
assistance = merge(assistance, folk, by=c('id','year'), all.x = TRUE)
print("Merge is done")
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 12) Expand datasets to create panels. ####
### ### ### ### ### ### ### ### ### ### ### ### ###
...
data_hosp = lapply(cols, function(x) {
  
  # Subset the data:
  data = df_hosp[dimension == x]
  
  # Expand it:
  setkey(setDT(data), municipality, year, month, group, 
         dimension, profession)
  combs = data[, CJ( unique(municipality), unique(year), unique(month), 
                     unique(group), unique(dimension), unique(profession))]
  data = data[combs][is.na(contacts), contacts := 0]
  setkey(data, NULL)
...
data_hosp = do.call(rbind.data.frame, data_hosp)
