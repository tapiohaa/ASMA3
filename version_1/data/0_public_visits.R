...
input_topi = "W:/ASMA3/data/raw/topi_unit_register_2022_06.csv"
input_sote = "W:/ASMA3/data/raw/sote_organisation_register.xlsx"
input_sote_new = "W:/ASMA3/data/raw/sote_2022_02.xlsx"
input_visits = "W:/ASMA3/data/interim/visits_20" # visits_20XX.csv, XX in 19:20

# Outputs:
output_visits = "W:/ASMA3/data/interim/visits_public_20" # visits_public_20XX.rds, XX in 19:20
...
# When extracting the primary care units, we observe from 258 to 238
# municipalities out of 293. Not every municipality has a health station or a 
# health center, but let's examine this closer.
...
sote_20 = setDT(readxl::read_excel(input_sote))
sote_22 = setDT(readxl::read_excel(input_sote_new))

sote = lapply(list(sote_20, sote_22), function(sote_df) {
  
  # Extract relevant columns:
  cols = c('Tunniste', 'Org.Yks.lyhenne', 
           'Hierarkiataso', 'Sektori', 'Sijainti kunta')
  DT = sote_df[, mget(cols)]
  setnames(DT, old=cols, new=c('sote_code', 'name', 
                                 'hierarchy', 'sector', 'municipality'))
  
  # Tidy the sector column:
  DT = DT[, sector := as.integer(substr(sector, 1, 1))]
  
  # For merging, keep only sote_code and sector:
  DT = DT[, mget(c('sote_code', 'sector'))]
  
})

# We will use sote_20 for 2019-2020 and sote_22 for 2021-2022:
sote_20 = sote[[1]]
sote_22 = sote[[2]]
sote_19 = sote_20[, mget(colnames(sote_20))][, year := 2019]
sote_20[, year := 2020]
sote_21 = sote_22[, mget(colnames(sote_22))][, year := 2021]
sote_22[, year := 2022]
sote = rbind(sote_19, sote_20, sote_21, sote_22)
...
visits = lapply(years, function(yr) {
  ...
  sote_yr = sote[year == 2000 + yr][, year := NULL]
  df = merge(df, sote_yr, by='sote_code', all.x = TRUE)
...
# All rows contain topi_code, but in 3.9% of the rows the linking did not work.
# 0.05% of the rows do not contain sote_code, and in 0.7% of the rows the
...
visits[, .N, by=c('year', 'pc_or_public')]
